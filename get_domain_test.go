...
func TestDNS1Cloud_GetDomain(t *testing.T) {
	testCases := []struct {
		name           string
		responseStatus int
		responseJSON   string
		...
		},
		{
			...
			expErrString:   "could not send command get_domain: could not unmarshal response: invalid character 'i' looking for beginning of value",
		},
		{
			name:           "incorrect json",
			responseStatus: http.StatusOK,
			responseJSON:   `["foo","bar"]`,
			expErrString:   "could not send command get_domain: could not unmarshal response: json: cannot unmarshal array into Go value of type dns1cloud.Domain",
		},
		{
			name:           "bad response",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				assert.Equal(t, "/dns/123", r.URL.Path)
			...
			domain, err := c.GetDomain(context.Background(), 123)
